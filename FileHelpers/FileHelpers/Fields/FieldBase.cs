...
namespace FileHelpers
{
    ...
    internal abstract class FieldBase
    {
        ...
        internal string TrimBeginCharacters(string field)
        {
            int ExtractedFrom = 0;
            int ExtractedTo = field.Length-1;
            while (ExtractedFrom <= ExtractedTo && Array.BinarySearch(mTrimChars, field[ExtractedFrom]) >= 0)
            {
                ExtractedFrom++;
            }
            ...
            if (ExtractedTo < ExtractedFrom)
                ...
                return field.Substring(ExtractedFrom, ExtractedTo - ExtractedFrom + 1);
        }

        internal string TrimEndCharacters(string field)
        {
            int ExtractedFrom = 0;
            int ExtractedTo = field.Length-1;
            while (ExtractedTo >= ExtractedFrom && Array.BinarySearch(mTrimChars, field[ExtractedTo]) >= 0)
            {
                ExtractedTo--;
            }
        ...
        internal string TrimBothCharacters(string field)
        {
            int ExtractedFrom = 0;
            int ExtractedTo = field.Length-1;
            while (ExtractedFrom <= ExtractedTo && Array.BinarySearch(mTrimChars, field[ExtractedFrom]) >= 0)
        ...
        internal string CreateFieldString(object fieldValue)
        {
            ...
            if (mConvertProvider == null)
            {
                if (fieldValue == null)
                    ...
                    rString = fieldValue.ToString();

            }
            else
            {
                rString = mConvertProvider.FieldToString(fieldValue);
            }
            ...
            switch (mTrimMode)
            {
                ...
                case TrimMode.Both:
                    rString = TrimBothCharacters(rString);
                    break;

                case TrimMode.Left:
                    rString = TrimBeginCharacters(rString);
                    break;

                case TrimMode.Right:
                    rString = TrimEndCharacters(rString);
                    break;
            }
        ...
        public object CreateValueForField(object fieldValue)
        {
            ...
            if (fieldValue == null)
            {
                if (mNullValue == null)
                {
                    if (mFieldTypeInternal.IsValueType && !mFieldTypeInternal.Name.Contains("Nullable"))
